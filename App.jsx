...
function deriveObjectif(o) {
  ...
  const roadPaces = computeRoadPaces(paceRefKm, paceRefSec);
  ...
  return { raceKmEq, raceRatio, startKmEq, startRatio, paceMinPerKmEq, ultraPaceMinPerKmEq, raceHours, raceHoursLow, raceHoursHigh, raceHoursSource, raceHoursSourceLabel,
    discipline, effectiveDiscipline, autoRoadLogic, strategy, ultraSpecific, roadPaces, goalAssessment, paceRefKm, paceRefSec, chocWeekendCapHours, referencePeakKmEq, picChocLikely, picChocWeeklyHours, wantedWeekendHours,
...
function selectTemplateFor(library, wantedType) {
  return pickTemplateByType(library, wantedType) || null;
}
...
function buildWeekAssignments(phase, trainingDaysPerWeek, library, rotIdx, weekend, week, objectif) {
  ...
  const place = (day, wantedType, hours, km, dplus) => {
    const tid = selectTemplateFor(library, wantedType);
    ...
    if (tid) { inst.templateId = tid; }
      ...
      const gen = generateSession(wantedType, hours * 60, km, dplus);
      generated.push(gen);
  ...
  const jeudiVMA = objectif && objectif.fractionneDuJeudi && !isChocWeek && phase !== 'recup' && phase !== 'course'
    && dayCapacityHours(availability, 'jeu') > 0;
  if (jeudiVMA) weekdaySlots.push({ day: 'jeu', type: 'Fractionné court', second: false, fixed: true });
  ...
  alloc.forEach(({ sl, hours }) => {
    ...
    place(sl.day, sl.type, hours, restKm * ratio, restDPlus * ratio);
  });
  ...
  weekendSlots.forEach(sl => {
    ...
    if (hours <= 0.08) return;
    placedHours += hours;
    place(sl.day, sl.type, hours, weekendKm * sl.share, weekendDPlus * sl.share);
  });
        ...
        {DAYS.map((d, idx) => (
          <div key={d.key} className="border border-stone-800 rounded-lg bg-stone-900 flex flex-col min-h-[140px]">
            ...
            <div className="flex-1 p-2 space-y-1.5">
              {(() => {
                const list = weekPlan[d.key] || [];
                const slotOrder = ['morning', 'midday', 'evening', 'free'];
                const used = slotOrder.filter(s => list.some(i => (i.slot || 'free') === s));
                // Only label slots when the day actually splits across more
                // than one — otherwise the labels are noise.
                return used.length > 1 ? used.map(s => (
                  <div key={s} className="text-[10px] uppercase tracking-wide text-stone-600 pt-1 first:pt-0">{SLOT_LABELS[s] || ''}</div>
                )) : null;
              })()}
              {(weekPlan[d.key] || []).slice().sort((a, b) => {
                const order = { morning: 0, midday: 1, evening: 2, free: 3 };
                return (order[a.slot || 'free'] ?? 3) - (order[b.slot || 'free'] ?? 3);
              }).map(inst => {
                const t = templateById(inst.templateId);
                if (!t) return null;
                const e = effective(inst, t);
                ...
                return (
                  <div key={inst.instanceId} className="border border-stone-800 rounded-md p-2 bg-stone-950">
                    ...
                    <div className="flex items-center flex-wrap gap-y-1">{inst.slot && inst.slot !== 'free' && <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-stone-800 text-stone-400 border border-stone-700 mr-1">{SLOT_LABELS[inst.slot]}</span>}<TypeTag type={t.type} /><DisciplineTag session={t} />{t.generated && <span className="inline-block text-[10px] px-1.5 py-0.5 rounded border ml-1 bg-stone-800 text-stone-400 border-stone-700">générée</span>}</div>
    ...
    <div className="border border-stone-800 rounded-lg bg-stone-900 p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-stone-500 mt-1">
          Les assistants utilisent l'API Anthropic. Sur cette version web, il faut ta propre clé.
          Sans clé, tout le reste de l'application fonctionne normalement.
        </p>
      </div>
      <div className="flex gap-2 flex-wrap items-end">
        <Field label="Clé API Anthropic">
          <input type="password" className={`${inputCls} w-72`} value={key} placeholder="sk-ant-..." onChange={e => setKey(e.target.value)} />
        </Field>
      ...
      <div className="text-[11px] text-amber-300 bg-amber-950 border border-amber-800 rounded px-3 py-2">
        ...
        Elle reste visible dans les outils de développement — acceptable pour un usage personnel, pas sur un appareil partagé.
        La consommation est facturée sur ton compte. Créer une clé : console.anthropic.com
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [generatedSessions, setGeneratedSessionsState] = useState({}); // { [weekKey]: weekPlan } — one storage key for every week, not one key per week
  ...
  const generatePlan = (objectifData, weeks) => {
    ...
    setAllPlansState(prev => {
      ...
      const genSessions = {};
      weeks.forEach((w, idx) => {
        const a = buildWeekAssignments(w.phase, objectifData.trainingDaysPerWeek || 5, library, idx, w.weekend || null, w, genObjectif);
