...
function simpleFieldsToStepInput(f) {
  ...
  else if (f.targetType === 'pace') target = { type: 'pace', lowKmh: paceToKmh(f.paceSlow), highKmh: paceToKmh(f.paceFast) };
...
const MACROCYCLES = {
  anticipation: { label: 'Anticipation', short: 'ANTIC.', color: '#8B7BD8' },
  developpement: { label: 'Développement', short: 'DÉVELOPPEMENT', color: '#5DCAA5' },
  specifique: { label: 'Spécifique', short: 'SPÉCIFIQUE', color: '#EF9F27' },
...
const DISCIPLINE_STRATEGIES = {
  ...
  route: {
    ...
    primaryMetric: 'km',
    longRunDay: 'dim',
    qualityTypes: ['Seuil', 'Fractionné long', 'Fractionné court'],
    phaseTypes: {
      anticipation: ['EF', 'Récupération'],
      developpement: ['EF', 'Fractionné court', 'Sortie longue', 'Récupération'],
      specifique: ['Seuil', 'Fractionné long', 'Sortie longue', 'EF'],
...
function deriveObjectif(o) {
  ...
  const SHORT_RACE_KMEQ = 55;
  const autoRoadLogic = discipline === 'trail' && raceKmEq > 0 && raceKmEq < SHORT_RACE_KMEQ;
  const effectiveDiscipline = (discipline === 'route' || autoRoadLogic) ? 'route' : 'trail';
  ...
  const recommendedPeakKmEq = Math.round(Math.max(lastWeekendKmEq / 0.80, raceKmEq * 0.80));
  ...
  return { raceKmEq, raceRatio, startKmEq, startRatio, paceMinPerKmEq, ultraPaceMinPerKmEq, raceHours, raceHoursLow, raceHoursHigh, raceHoursSource, raceHoursSourceLabel,
    discipline, effectiveDiscipline, autoRoadLogic, strategy, ultraSpecific, chocWeekendCapHours, referencePeakKmEq, picChocLikely, picChocWeeklyHours, wantedWeekendHours,
    maxHoursPerDay, specificity, lastWeekendHoursTarget, lastWeekendKmEq, recommendedPeakKmEq, peakKmEq, habitualKmEq, suggestedTier, effectiveTier };
}
...
function buildPeriodizationPlan(objectif) {
  ...
  const totalWeeks = Math.round((raceMonday - startMonday) / (7 * 24 * 3600 * 1000));
  ...
  const taperWeeks = Math.max(0, Math.min(D.picChocLikely ? Math.max(requestedTaper, 3) : requestedTaper, totalWeeks));
  ...
  // on its own.
  const weeksNeeded = weeksToReachPeak(D.startKmEq, D.peakKmEq, objectif.trainingDaysPerWeek || 5, tier) + taperWeeks;
  ...
  for (let i = 0; i <= totalWeeks; i++) {
    ...
    if (weeksBeforeRace === 0) {
      ...
      phase = cyclePos === 0 ? 'reprise' : cyclePos === 1 ? 'charge' : cyclePos === 2 ? 'choc' : 'recup';
      weekKmEq = kmEq;
    }
...
function selectTemplateFor(library, wantedType, macro, strategy) {
  let tid = pickTemplateByType(library, wantedType);
  if (tid) return tid;
  const prefs = (strategy.phaseTypes && strategy.phaseTypes[macro]) || [];
  for (const t of prefs) { tid = pickTemplateByType(library, t); if (tid) return tid; }
  return pickTemplateByType(library, 'EF') || (library[0] && library[0].id) || null;
}
...
function qualityTypeFor(macro, strategy, library, rotIdx) {
  ...
  const avail = prefs.filter(t => library.some(l => l.type === t) && t !== 'EF' && t !== 'Récupération' && t !== 'Sortie longue');
...
function buildWeekAssignments(phase, trainingDaysPerWeek, library, rotIdx, weekend, week, objectif) {
  ...
  if (isChocWeek && weekend) {
    ...
    weekendDays.forEach(day => {
      ...
      weekendSlots.push({ day, share, type: day === longDay ? 'Sortie longue' : 'EF' });
    });
  }

  // ---- 2. What's left for the weekdays ------------------------------------
  const restHours = Math.max(0, weekHours - weekendHours);
  ...
  // How many weekday sessions we're allowed to place. Doubles only when the
  // plan genuinely needs them AND this week is heavy enough to justify one.
  const heavyWeek = restHours > available.length * MAX_SESSION_HOURS * 0.85;
  const allowDoubles = doubles.required && doubles.possible && (macro === 'specifique' || macro === 'anticipation' ? true : heavyWeek);
  // Adaptation doubles during Développement: light, occasional, never a way to
  // manufacture a huge week.
  const adaptationDouble = doubles.required && doubles.possible && macro === 'developpement' && !heavyWeek && (rotIdx % 2 === 0);

  const daySlotsPlan = {};
  available.forEach(k => { daySlotsPlan[k] = 1; });
  if (allowDoubles || adaptationDouble) {
    const capable = available.filter(k => daySlotCount(availability, k) >= 2 && dayCapacityHours(availability, k) > MAX_SESSION_HOURS);
    const wanted = adaptationDouble ? 1 : Math.max(1, Math.ceil((restHours - available.length * MAX_SESSION_HOURS) / MAX_SESSION_HOURS));
    capable.slice(0, wanted).forEach(k => { daySlotsPlan[k] = 2; });
  }

  // Respect the requested number of training days where possible: trim the
  // least useful days rather than cramming everything in.
  ...
  const keptDays = orderedDays.slice(0, Math.max(1, Math.min(orderedDays.length, wantedWeekdaySessions || orderedDays.length)));

  // ---- 3. Build the weekday session list ----------------------------------
  const qType = qualityTypeFor(macro, strategy, library, rotIdx);
  const stripQuality = isChocWeek || phase === 'recup' || phase === 'course';
  const weekdaySlots = [];
  keptDays.forEach((day, i) => {
    const n = daySlotsPlan[day] || 1;
    for (let s = 0; s < n; s++) {
      let type;
      if (s > 0) type = 'EF';                                   // second session stays easy
      else if (!stripQuality && qType && i === 0) type = qType;  // one quality session
  ...
  weekdaySlots.forEach(sl => {
    const share = weightOf(sl) / totalWeight;
    let hours = restHours * share;
    // Session ceiling, then what remains of the day's ceiling.
    hours = Math.min(hours, MAX_SESSION_HOURS);
    const used = dayUsed[sl.day] || 0;
    hours = Math.min(hours, Math.max(0, dayCapacityHours(availability, sl.day) - used));
    if (hours <= 0.08) return;
    dayUsed[sl.day] = used + hours;
    placedHours += hours;
    const tid = selectTemplateFor(library, sl.type, macro, strategy);
    if (!tid) return;
    const ratio = restHours > 0 ? hours / restHours : 0;
    assign[sl.day].push({
      instanceId: uid(), templateId: tid,
      overrideKm: Math.round(restKm * ratio * 10) / 10,
      overrideDPlus: Math.round(restDPlus * ratio),
      overrideMin: Math.round(hours * 60),
    });
  });

  // ---- 5. Weekend sessions -------------------------------------------------
  weekendSlots.forEach(sl => {
    const hours = weekendHours * sl.share;
    if (hours <= 0.08) return;
    const tid = selectTemplateFor(library, sl.type, macro, strategy);
    if (!tid) return;
    placedHours += hours;
    assign[sl.day].push({
      instanceId: uid(), templateId: tid,
      overrideKm: Math.round(weekendKm * sl.share * 10) / 10,
      overrideDPlus: Math.round(weekendDPlus * sl.share),
      overrideMin: Math.round(hours * 60),
    });
  });
  ...
  assign.__meta = {
    ...
    capacityExceeded: weekHours - placedHours > 0.4,
  };
  return assign;
}
...
// ---------------------------------------------------------------------------
// AI access on the public web build.
//
// In the Claude artifact, requests to the Anthropic API were proxied by the
// host. A static site has no such proxy, so the user must supply their own API
// key (stored locally, never sent anywhere except Anthropic).
//
// Honest caveat shown in the UI: a browser-side key is visible in devtools and
// in the network tab. It is fine for personal use, not for a shared instance.
// ---------------------------------------------------------------------------
function getApiKey() {
  try { return window.localStorage.getItem('goat-trail:api-key') || ''; } catch { return ''; }
}
function setApiKey(k) {
  try { k ? window.localStorage.setItem('goat-trail:api-key', k) : window.localStorage.removeItem('goat-trail:api-key'); } catch { /* ignore */ }
}
...
async function callClaude(body) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Erreur API (${response.status}). ${txt.slice(0, 200)}`);
  }
...
// ============================================================================
// Persistence. In the Claude artifact this used a host-provided API; on the
// web we back it with localStorage. Same async contract, so nothing else in
...
async function storageSet(key, value) {
  try { window.localStorage.setItem(STORAGE_PREFIX + key, JSON.stringify(value)); } catch { /* quota or private mode */ }
}
        ...
        {library.map(t => editingId === t.id ? (
          ...
          <div key={t.id} className="border border-stone-800 rounded-lg p-3 bg-stone-900 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-stone-500 mt-0.5">{t.distanceKm} km · {t.dureeMin} min · {t.deniveleM || 0} m D+ · {t.blocks.length} bloc(s)</div>
            </div>
...
function PlanningTab({ library, monday, setMonday, weekPlan, setWeekPlan, meta, onOpenSession }) {
        ...
        {DAYS.map((d, idx) => (
          <div key={d.key} className="border border-stone-800 rounded-lg bg-stone-900 flex flex-col min-h-[140px]">
            ...
            <div className="flex-1 p-2 space-y-1.5">
              {(weekPlan[d.key] || []).map(inst => {
                const t = templateById(inst.templateId);
                if (!t) return null;
                const e = effective(inst, t);
                const scaled = inst.overrideKm != null;
                return (
                  <div key={inst.instanceId} className="border border-stone-800 rounded-md p-2 bg-stone-950">
                    <div className="flex items-start justify-between gap-1">
                      <button onClick={() => onOpenSession && onOpenSession(t.id)} className="text-left flex-1 min-w-0 group">
                        ...
                        <span className="text-[11px] text-stone-500 block truncate">{t.nom} · {sessionQualifiers(t, e)}</span>
                      </button>
                      <button onClick={() => removeFromDay(d.key, inst.instanceId)} className="text-stone-600 hover:text-red-400 shrink-0"><X size={14} /></button>
                    </div>
                    <TypeTag type={t.type} />
                    <div className="text-xs text-stone-500 mt-1">{e.km} km · {e.min} min · {e.dplus} m D+</div>
...
// Web-build only: let the user supply their own Anthropic key to enable the
// AI chat features, with an honest note about what that implies.
    ...
    <div className="border border-stone-800 rounded-lg bg-stone-900 p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-stone-500 mt-1">
          Les assistants (remplir un objectif, saisir ton historique, planifier une semaine) utilisent l'API Anthropic.
          Sur cette version web, il faut ta propre clé. Sans clé, tout le reste de l'application fonctionne normalement.
        </p>
      </div>
      <div className="flex gap-2 flex-wrap items-end">
        <Field label="Clé API Anthropic">
          <input type="password" className={`${inputCls} w-72`} value={key} placeholder="sk-ant-..."
      ...
      <div className="text-[11px] text-amber-300 bg-amber-950 border border-amber-800 rounded px-3 py-2">
        À savoir : la clé est stockée dans ton navigateur et envoyée directement à Anthropic depuis la page.
        Elle reste visible dans les outils de développement — c'est acceptable pour un usage personnel, mais n'utilise
        pas cette page sur un appareil partagé, et ne partage pas ta clé. La consommation est facturée sur ton compte.
        Tu peux créer une clé sur console.anthropic.com.
      </div>
    </div>
  );
}
...
function initialObjectifState(o) {
  return {
    ...
    cutoffH: o.cutoffH || '', raceHoursEstimate: o.raceHoursEstimate || '',
          ...
          <div className="max-h-96 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="bg-stone-950 text-stone-500 text-xs sticky top-0"><tr>
                ...
                <th className="text-left px-3 py-1.5 font-medium">D+</th>
                <th className="text-left px-3 py-1.5 font-medium">Temps</th>
                <th className="text-left px-3 py-1.5 font-medium hidden sm:table-cell">km-éq</th>
                <th className="text-left px-3 py-1.5 font-medium hidden sm:table-cell">Ratio</th>
              </tr></thead>
              <tbody>
                {preview.map(w => (
                  <tr key={w.weekKey} onClick={() => setSelectedWeekKey(w.weekKey)}
                    ...
                    <td className="px-3 py-1.5 text-stone-200 font-medium">{w.targetDPlus}</td>
                    <td className="px-3 py-1.5 text-stone-200 font-medium">{fmtHoursLabel(w.targetHours)}</td>
                    <td className="px-3 py-1.5 text-stone-500 hidden sm:table-cell">{w.targetKmEq}</td>
                    <td className="px-3 py-1.5 text-stone-500 hidden sm:table-cell">{w.ratio}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ObjectifTab({ objectif, onGenerate, library, profile, onGoToProfil }) {
  ...
  const missingTypes = SEANCE_TYPES.filter(t => t !== 'Autre' && !library.some(l => l.type === t));
  ...
  if (raceDateObj && D.peakKmEq) {
    const weeksNeeded = weeksToReachPeak(D.startKmEq, D.peakKmEq, parsed.trainingDaysPerWeek, effectiveTier) + parsed.taperWeeks;
    ...
    tierComparison = Object.keys(TIER_LABELS).map(t => {
      const weeks = weeksToReachPeak(D.startKmEq, referencePeak, parsed.trainingDaysPerWeek, t) + referenceTaper;
    ...
    <div className="space-y-4 max-w-4xl lg:max-w-none">
      ...
      {fillMode === 'sequential' && (
        <>
          ...
          {qIndex === 3 && (
            <QuestionShell qIndex={3} total={TOTAL_Q} onCancelAll={cancelSequential} title="Distance et dénivelé ?" subtitle={D.raceKmEq > 0 ? `≈ ${Math.round(D.raceKmEq)} km-éq · ratio ${Math.round(D.raceRatio)} m D+/km-éq` : null} onNext={next} onBack={back} nextDisabled={!d.distanceKm}>
          ...
          {qIndex === 5 && (
            <QuestionShell qIndex={5} total={TOTAL_Q} onCancelAll={cancelSequential} title="Un temps visé, ou un cut-off ?" subtitle={`Facultatif — sinon on l'estime à ${formatDuration(D.raceHoursLow*60)}-${formatDuration(D.raceHoursHigh*60)} (${D.raceHoursSource === 'historique' ? `d'après ton historique` : `palier ${TIER_LABELS[effectiveTier]}`}).`} onNext={next} onBack={back} nextDisabled={false} canSkip onSkip={next}>
              <div className="flex gap-3">
                <Field label="Temps visé (h)"><input type="number" inputMode="decimal" step="0.5" className={`${inputCls} w-full text-lg py-3`} value={d.raceHoursEstimate} onChange={e => setD({ ...d, raceHoursEstimate: e.target.value })} /></Field>
                <Field label="Cut-off (h)"><input type="number" inputMode="decimal" className={`${inputCls} w-full text-lg py-3`} value={d.cutoffH} onChange={e => setD({ ...d, cutoffH: e.target.value })} /></Field>
              </div>
            </QuestionShell>
          )}
          {qIndex === 6 && (
            <QuestionShell qIndex={6} total={TOTAL_Q} onCancelAll={cancelSequential} title="Ton volume de départ souhaité ?" subtitle={`≈ ${Math.round(D.startKmEq)} km-éq · ratio ${Math.round(D.startRatio)} m D+/km-éq`} onNext={next} onBack={back} nextDisabled={false}>
              <div className="flex gap-3">
          ...
          {qIndex === 8 && (
            <QuestionShell qIndex={8} total={TOTAL_Q} onCancelAll={cancelSequential} title="Sur combien de jours par semaine ?" onNext={next} onBack={back} nextDisabled={false}>
              <div className="flex gap-2 flex-wrap">
                {[3, 4, 5, 6, 7].map(n => (
      ...
      {planInfo.length > 0 && (
        <div className="text-xs bg-stone-900 border rounded px-3 py-2 space-y-2" style={{ borderColor: PHASE_COLORS.picchoc.bar + '66', color: PHASE_COLORS.picchoc.text }}>
          {planInfo.map((p, i) => (
            <div key={i}><div className="font-medium flex items-center gap-1.5"><Flame size={13} />{p.title}</div><div className="mt-0.5 text-stone-300">{p.text}</div></div>
          ))}
        </div>
      )}
      ...
      <details className="border border-stone-800 rounded-lg bg-stone-900 group">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            <Field label="Jours d'entraînement/sem"><input type="number" inputMode="decimal" min="2" max="7" className={`${inputCls} w-20`} value={d.trainingDaysPerWeek} onChange={e => setD({ ...d, trainingDaysPerWeek: e.target.value })} /></Field>
...
export default function App() {
  ...
  const [library, setLibrary] = useState(ULTRA_LIBRARY);
  const [monday, setMonday] = useState(() => getMonday(new Date()));
  const [allPlans, setAllPlansState] = useState({});
  const [weekMeta, setWeekMetaState] = useState({});
  const [focusSessionId, setFocusSessionId] = useState(null); // { [weekKey]: weekPlan } — one storage key for every week, not one key per week
  ...
  useEffect(() => {
    (async () => {
      ...
      const lib = libVersion >= 4 ? await storageGet('seance-library', null) : null;
      if (libVersion < 4) { storageSet('library-version', 4); storageSet('seance-library', ULTRA_LIBRARY); }
  ...
  const generatePlan = (objectifData, weeks) => {
    ...
    storageSet('race-objectif', objectifData);
    ...
    setAllPlansState(prev => {
      const next = { ...prev };
      weeks.forEach((w, idx) => {
        next[w.weekKey] = buildWeekAssignments(w.phase, objectifData.trainingDaysPerWeek || 5, library, idx, w.weekend || null, w, genObjectif);
      });
    ...
    weeks.forEach(w => {
      ...
      meta[w.weekKey] = { phase: w.phase, macrocycle: w.macrocycle, note: w.note || null, atPeak: !!w.atPeak,
        ...
        capacityExceeded: am.capacityExceeded };
    });
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-4 lg:pl-52 lg:max-w-[1400px]">
        ...
        {!ready ? (
          ...
          <>
            ...
            {tab === 'planning' && (
              <PlanningTab library={library} monday={monday} setMonday={setMonday} weekPlan={weekPlan} setWeekPlan={setWeekPlan} meta={weekMeta[weekKey(monday)] || null} onOpenSession={(id) => { setFocusSessionId(id); setTab('library'); }} />
            )}
